...
howToSteps:
  ...
  - name: "Pull all 5 exit paths"
    text: "Pull all five exit paths, not just the cash-buyer postcard option: traditional listing with a senior-specialist agent, subject-to (Sub2), owner financing, lease option, or as-is cash sale to an investor. The math on options 1 through 4 is almost always better when the family has time."
  - name: "Get an independent read"
    text: "Get an independent read from a neutral broker or Blueprint walkthrough before pen hits paper. The cash buyer and the agent who wants the listing are both interested parties, neither will lay out every option side by side. RSS has zero financial interest in which direction the family goes."
...
**Coordinate your family in one place.** SeniorSafeApp app (web, iPhone, Android): app.seniorsafeapp.com
